...
`;
    ...
    weiboButton.onclick = function () {
        // weiboButton的第三个兄弟节点是复制按钮，选它点击即可
...
function addWeiboButtons() {
    // 获取所有带有 "icon-md-heavy" 类的元素
    const iconElements = document.getElementsByClassName('icon-md-heavy');

    Array.from(iconElements).forEach(iconElement => {
        // 获取该元素的父节点div
        const parentDiv = iconElement.closest('div.flex.items-center');

        if (parentDiv && !processedDivs.has(parentDiv)) {
            console.log("Weibo Button added", parentDiv);
...
function observeChat() {
    ...
    if (conversationContainer) {
        const observer = new MutationObserver(() => {
            console.log("New conversation found, adding Weibo buttons...");
            addWeiboButtons();
        });

        observer.observe(conversationContainer, { childList: true, subtree: true });
        console.log("MutationObserver is now observing the chat container.");
    } else {
